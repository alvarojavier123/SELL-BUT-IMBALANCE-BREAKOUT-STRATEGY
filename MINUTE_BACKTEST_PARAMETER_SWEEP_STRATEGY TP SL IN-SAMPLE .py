...
stop_loss_list = [0.005, 0.01, 0.015, 0.02]
take_profit_list = [0.05, 0.1, 0.15, 0.2]
    ...
    for take_profit_pct in take_profit_list:
        for lookback in lookback_list:
            for holding_period in holding_period_list:
                print(Fore.YELLOW + f"\n==== L{lookback}-H{holding_period}-SL{stop_loss_pct}-TP{take_profit_pct} ====" + Style.RESET_ALL)
                ...
                holding_period_minutes = holding_period * 60
                last_exit = -1
                trades = []

                for i in tqdm(range(len(min_df) - holding_period_minutes), desc=f"L{lookback}-H{holding_period}-SL{stop_loss_pct}-TP{take_profit_pct}"):
                ...
                label = f"L{lookback}-H{holding_period}-SL{stop_loss_pct}-TP{take_profit_pct}"
                ...
                trade_log.to_csv(f"minute_backtest_outputs/trades_L{lookback}_H{holding_period}_SL{stop_loss_pct}_TP{take_profit_pct}.csv")

summary_df = pd.DataFrame(summary)
summary_df.to_csv("SUMMARY_SWEEP_LOOKBACK_HOLD_SL_TP.csv")
